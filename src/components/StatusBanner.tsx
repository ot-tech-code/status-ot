...
export const StatusBanner: React.FC<StatusBannerProps> = ({ summary, siteCount }) => {
  ...
  let bgClass = 'bg-emerald-50 dark:bg-emerald-950/40 border-emerald-200 dark:border-emerald-500/30 text-emerald-900 dark:text-emerald-200';
  let icon = <CheckCircle2 className="w-7 h-7 text-emerald-600 dark:text-emerald-400" />;
  ...
  if (overallStatus === 'outage') {
    bgClass = 'bg-rose-50 dark:bg-rose-950/40 border-rose-200 dark:border-rose-500/30 text-rose-900 dark:text-rose-200';
    icon = <XCircle className="w-7 h-7 text-rose-600 dark:text-rose-400" />;
    ...
    bgClass = 'bg-amber-50 dark:bg-amber-950/40 border-amber-200 dark:border-amber-500/30 text-amber-900 dark:text-amber-200';
  ...
  return (
    <div className={`rounded-xl border p-5 sm:p-6 shadow-sm dark:shadow-lg backdrop-blur-sm transition-all ${bgClass}`}>
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div className="flex items-start space-x-4">
          <div className="p-2 rounded-lg bg-white/80 dark:bg-slate-900/60 border border-slate-200 dark:border-slate-800 shrink-0">
            {icon}
          </div>
          <div>
            <h1 className="text-xl sm:text-2xl font-bold tracking-tight text-slate-900 dark:text-white">{title}</h1>
            <p className="text-sm mt-1 text-slate-600 dark:text-slate-300 max-w-2xl">{subtitle}</p>
          </div>
        </div>

        <div className="flex items-center space-x-4 text-xs font-mono bg-white/90 dark:bg-slate-900/80 px-4 py-2.5 rounded-lg border border-slate-200 dark:border-slate-800/80 text-slate-700 dark:text-slate-300 shrink-0 shadow-sm">
