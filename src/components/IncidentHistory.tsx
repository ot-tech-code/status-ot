    ...
    <div className="space-y-6">
      ...
      {incidents.length === 0 ? (
        <div className="bg-slate-900 border border-slate-800 rounded-xl p-12 text-center">
          ...
          <h3 className="text-lg font-bold text-white">No Active or Past Incidents</h3>
            ...
            return (
              <div
                ...
                className={`border rounded-xl p-5 transition-all bg-slate-900 ${
                  isResolved ? 'border-slate-800' : 'border-rose-500/40 bg-rose-950/20'
                }`}
              >
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
                  <div className="flex items-start space-x-3">
                    <div className={`p-2 rounded-lg shrink-0 mt-0.5 ${
                      isResolved ? 'bg-slate-800 text-slate-300' : 'bg-rose-500/10 text-rose-400 border border-rose-500/30'
                    }`}>
                      {isResolved ? <CheckCircle2 className="w-5 h-5 text-emerald-400" /> : <AlertOctagon className="w-5 h-5" />}
                    </div>

                    <div>
                      <div className="flex items-center space-x-2">
                        <h4 className="text-base font-bold text-white">{incident.siteName}</h4>
                        <span className={`px-2 py-0.5 text-[10px] font-bold rounded ${
                          isResolved ? 'bg-emerald-500/10 text-emerald-400 border border-emerald-500/20' : 'bg-rose-500/20 text-rose-300 border border-rose-500/30'
                        }`}>
                          {isResolved ? 'RESOLVED' : 'ACTIVE OUTAGE'}
                        </span>
                      </div>

                      <p className="text-xs font-mono text-slate-400 mt-0.5">{incident.url}</p>
                      
                      {incident.error && (
                        <p className="text-xs font-mono text-rose-300 bg-slate-950 px-2.5 py-1.5 rounded border border-slate-800 mt-2">
                  ...
                  <div className="text-right text-xs font-mono text-slate-400 space-y-1 shrink-0">
